...
document.addEventListener("DOMContentLoaded",()=>{
    ...
    /* === INIT === */
    (async()=>{
      await preloadBGs(); buildUI();
      overlay.querySelector("#bg-overlay").onclick = changeBg;
      overlay.querySelector("#save-overlay").onclick = ()=>savePng(true); // true = pobierz na dysk
      addModelListeners(); changeBg(); setLang(lang);
    })();
    
    /* ... (reszta funkcji bez zmian: preloadBGs, loadSvg, buildUI, setLang, itd.) ... */
    function preloadBGs(){BG.forEach(src=>{const i=new Image();i.src=src;});}
    async function loadSvg(){if(!currentSvg)return;gunBox.innerHTML=await fetch(currentSvg).then(r=>r.text());gunBox.appendChild(overlay);const svg=gunBox.querySelector("svg");const layer=document.createElementNS("http://www.w3.org/2000/svg","g");layer.id="color-overlays";svg.appendChild(layer);PARTS.filter(p=>!p.disabled).forEach(p=>{const base=svg.querySelector("#"+p.id);if(!base)return;["1","2"].forEach(n=>{const ov=base.cloneNode(true);ov.id=`color-overlay-${n}-${p.id}`;ov.classList.add("color-overlay");layer.appendChild(ov);});});setLang(lang);}
    function buildUI(){PARTS.forEach(p=>{const b=document.createElement("button");b.textContent=p[lang];b.dataset.id=p.id;if(p.disabled){b.classList.add("disabled");b.disabled=true;}else b.onclick=()=>selectPart(b,p.id);partsBox.appendChild(b);});['mix1','mix2'].forEach((id,i)=>{const m=document.createElement("button");m.className="mix";m.id=id;m.onclick=()=>mix(i?undefined:2);partsBox.appendChild(m);});Object.entries(COLORS).forEach(([full,hex])=>{const[code,...rest]=full.split(" ");const name=rest.join(" ");const sw=document.createElement("div");sw.className="sw";sw.title=full;sw.onclick=()=>applyColor(activePart,hex,code);sw.innerHTML=`<div class="dot" style="background:${hex}"></div><div class="lbl">${code}<br>${name}</div>`;palette.appendChild(sw);});resetBtn.onclick=resetAll;sendBtn.onclick=()=>modal.classList.remove("hidden");mCancel.onclick=()=>modal.classList.add("hidden");mSend.onclick=sendMail;if(langPl)langPl.onclick=()=>setLang("pl");if(langEn)langEn.onclick=()=>setLang("en");}
    function setLang(l){lang=l;localStorage.setItem('lang',l);document.title=l==="pl"?"Weapon-Wizards – Pistolet":"Weapon-Wizards – Pistol";const loadingText=document.getElementById('loading-text');if(loadingText)loadingText.textContent=l==='pl'?'Ładowanie...':'Loading...';partsBox.querySelectorAll("button").forEach(b=>{const p=PARTS.find(x=>x.id===b.dataset.id);if(p)b.textContent=p[lang];});const mix1=document.getElementById('mix1');if(mix1)mix1.textContent=l==='pl'?'MIX (≤2)':'MIX (≤2)';const mix2=document.getElementById('mix2');if(mix2)mix2.textContent=l==='pl'?'MIX (3+)':'MIX (3+)';hParts.textContent=l==="pl"?"1. Wybierz część":"1. Select part";hCol.textContent=l==="pl"?"2. Wybierz kolor (Cerakote)":"2. Select colour (Cerakote)";if(viewBtn)viewBtn.textContent=l==="pl"?"Zmień widok":"Change view";if(weaponBtn)weaponBtn.textContent=l==="pl"?"Zmień broń":"Change weapon";resetBtn.textContent=l==="pl"?"Resetuj kolory":"Reset colours";sendBtn.textContent=l==="pl"?"Wyślij do Wizards!":"Send to Wizards!";const bgOverlay=document.getElementById("bg-overlay");const saveOverlay=document.getElementById("save-overlay");if(bgOverlay)bgOverlay.textContent=l==="pl"?"Zmień tło":"Change background";if(saveOverlay)saveOverlay.textContent=l==="pl"?"Zapisz obraz":"Save image";mSend.textContent=l==="pl"?"Wyślij":"Send";mCancel.textContent=l==="pl"?"Anuluj":"Cancel";mName.placeholder=l==="pl"?"Imię":"Name";mMail.placeholder=l==="pl"?"E-mail":"E-mail";mPhone.placeholder=l==="pl"?"Telefon":"Phone";modalTitle.textContent=l==="pl"?"Wyślij projekt":"Send project";modalNote.textContent=l==="pl"?"Twój projekt zostanie wysłany automatycznie.":"Your project will be sent automatically.";langPl.classList.toggle("active",l==="pl");langEn.classList.toggle("active",l==="en");updateSummary();updatePrice();}
    function selectPart(btn,id){partsBox.querySelectorAll("button").forEach(b=>b.classList.remove("selected"));btn.classList.add("selected");activePart=id;}
    function applyColor(id,hex,code){if(!id){alert(lang==="pl"?"Najpierw wybierz część":"Select a part first");return;}["1","2"].forEach(n=>{const ov=document.getElementById(`color-overlay-${n}-${id}`);if(ov)(ov.tagName==="g"?ov.querySelectorAll("*"):[ov]).forEach(s=>s.style.fill=hex);});selections[id]=code;updateSummary();updatePrice();}
    function mix(maxCols){const keys=Object.keys(COLORS),used=new Set();PARTS.filter(p=>!p.disabled).forEach(p=>{let pick;do{pick=keys[Math.floor(Math.random()*keys.length)];}while(maxCols&&used.size>=maxCols&&!used.has(pick.split(" ")[0]));used.add(pick.split(" ")[0]);applyColor(p.id,COLORS[pick],pick.split(" ")[0]);});}
    function resetAll(){document.querySelectorAll(".color-overlay").forEach(o=>{(o.tagName==="g"?o.querySelectorAll("*"):[o]).forEach(s=>s.style.fill="transparent");});selections={};activePart=null;updateSummary();updatePrice();}
    ...
    function updateSummary(){const list=$("summary-list");list.innerHTML="";PARTS.forEach(p=>{if(selections[p.id]){const d=document.createElement("div");d.textContent=`${p[lang]} – ${selections[p.id]}`;list.appendChild(d);}});}
    function updatePrice(){const cols=new Set(Object.values(selections)).size;let total=Object.keys(selections).reduce((s,id)=>s+(PRICE[id]||0),0);total=cols<=2?Math.min(total,MIX2):Math.min(total,MIXN);priceBox.innerHTML=(lang==="pl"?"Szacowany koszt:&nbsp;&nbsp;":"Estimated cost:&nbsp;&nbsp;")+total+"&nbsp;zł";return total;}
    ...
    function chooseModel(model){const overlay=document.getElementById("model-select");if(overlay)overlay.classList.add("hidden");currentSvg=MODELS[model]||"g17.svg";if(model==="cz"){BG=BG_CZ;}else{BG=BG_DEFAULT;}bgIdx=0;changeBg();loadSvg();}

    /* === ZMODYFIKOWANA FUNKCJA ZAPISU OBRAZKA === */
    const loadImg=s=>new Promise(r=>{const i=new Image();i.onload=()=>r(i);i.src=s;});
    async function savePng(download = false){ // Parametr 'download' decyduje o akcji
      ...
      if (download) { // Jeśli true, pobierz plik na dysk klienta
        const a=document.createElement("a");a.href=cvs.toDataURL("image/png");a.download="weapon-wizards.png";a.click();
      } else { // W przeciwnym razie, zwróć dane obrazka do wysyłki
        return cvs.toDataURL("image/png");
      }
    }
    
    /* === OSTATECZNA FUNKCJA WYSYŁANIA MAILA === */
    async function sendMail(){
      ...
      try {
        ...
        PARTS.forEach(p => {
          if (selections[p.id]) {
            summaryText += `${p[lang]} – ${selections[p.id]}\n`;
          }
        });
        ...
        if (result.status === 'success') {
            ...
            setTimeout(() => {
                ...
                mSend.textContent = originalBtnText; // Przywróć tekst dopiero po zamknięciu
                mSend.disabled = false;
            }, 2000); // Zamknij okno po 2 sekundach
        } else {
        ...
        mSend.textContent = originalBtnText; // Przywróć od razu w razie błędu
        mSend.disabled = false;
      }
    }
});